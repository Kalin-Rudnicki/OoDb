...
class IoManager(path: String) {
  ...
  if (order < BTree.MIN_ORDER)
    throw new RuntimeException(
      s"Something has gone seriously wrong... (order: $order)"
    )
  ...
  def showStats: Unit = {
    ...
    while (bTreeFile.getFilePointer < bTreeFile.length) {
      ...
      val prefix =
        s"> ${pos.toString.reverse.padTo(10, ' ').reverse} / 0x${pos.toHexString.toUpperCase.reverse.padTo(8, '0').reverse}"
      t match {
        ...
        case LEAF_NODE_MARKER =>
          println(
            s"$prefix => LeafNode($keys, ${values.reverse.tail.reverse}, ${values.last})"
          )
      }
    }
    println("=====| EOF |=====")
  ...
  def writeImage(path: String, message: Option[String] = None): Unit = {
    ...
    message match {
      ...
      case Some(msg) =>
        ...
        graphics.drawString(msg, (w - fm2.stringWidth(msg)) / 2, tH2)
    }
  ...
  def insertNewNode(node: Node[_]): Unit = {
    val (insertAt, popFree): (Long, Boolean) =
      freeListHead.fold((bTreeFile.length, false))(f => (f, true))

    if (node.pos != insertAt)
      throw new RuntimeException(
        "Attempt to insert node new node with invalid 'pos', not at start of free list"
      )
  ...
  def deleteNode(node: Node[_], zeroOut: Boolean = false): Unit = {
    ...
    if (zeroOut)
      bTreeFile.writeBytable[(Byte, Long, List[Long])](
        (FREE_LIST_MARKER, freeListHead.getOrElse(0L), List[Long]())
      )(
      ...
      bTreeFile.writeBytable[(Byte, Long)](
        (FREE_LIST_MARKER, freeListHead.getOrElse(0L))
      )(Bytable.tuple2Bytable(Bytable.byteBytable, Bytable.longBytable))
