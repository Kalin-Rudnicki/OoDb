...
class BTree(private val io: IoManager) {
  ...
  import BTree.{ParentChange => P, RemoveResult => R}
  ...
  def insert(key: Long, value: Long): Unit = {
    // Helpers
    def handleInsert(
      res: Option[(Node[_], Option[(Long, Node[_])])]
    ): Option[(Long, Long)] = res match {
      case None =>
        None
      case Some((n1, None)) =>
        io.writeNode(n1)
      ...
      loop(1, io.getRoot) match {
        ...
        case Some((rightMin, pos)) =>
          val newRoot: InternalNode =
            InternalNode(io.nextFreePos, List(rightMin), List(io.getRoot, pos))
          io.insertRootNode(newRoot)
      } else {
      val node: LeafNode =
        new LeafNode(io.nextFreePos, List(key), List(value), 0L)
      io.setSize(1)
                  ...
                  (left, right) match {
                    case (Some(l), _) =>
                      ...
                      if (lNode.keys.length > io.minKeys) { // Borrow Left
                        ...
                        val lNode2 = InternalNode(
                          lNode.pos,
                          lNode.keys ::: minKey.get :: n1.keys,
                          lNode.children ::: n1.children
                        )
                        io.writeNode(lNode2)
                        io.deleteNode(n1)
                    ...
                    case (_, Some((rKey, rVal))) =>
                      ...
                      if (rNode.keys.length > io.minKeys) { // Borrow Right
                        ...
                        val n2 = InternalNode(
                          n1.pos,
                          n1.keys ::: rKey :: Nil,
                          n1.children ::: rNode.children.head :: Nil
                        )
                        io.writeNode(rNode2)
                        io.writeNode(n2)
                        ...
                        val n2 = InternalNode(
                          n1.pos,
                          n1.keys ::: rKey :: rNode.keys,
                          n1.children ::: rNode.children
                        )
                        io.writeNode(n2)
                        io.deleteNode(rNode)
              ...
              (left, right) match {
                ...
                case (_, Some((rKey, rVal))) =>
                  ...
                  if (rNode.keys.length > io.minKeys) { // Borrow Right
                    ...
                    val n2 = LeafNode(
                      n1.pos,
                      n1.keys ::: rNode.keys.head :: Nil,
                      n1.values ::: rNode.values.head :: Nil,
                      n1.nextLeaf
                    )
                    io.writeNode(rNode2)
                    io.writeNode(n2)
...
object BTree {
  ...
  def create(order: Int, path: String): BTree = {
    if (order < MIN_ORDER)
      throw new IllegalArgumentException(
        s"order $order is less than min $MIN_ORDER"
      )
  ...
  object RemoveResult {
    ...
    case class Cascade(v: Long, changes: MMap[Long, ParentChange])
        extends Action(v) {
