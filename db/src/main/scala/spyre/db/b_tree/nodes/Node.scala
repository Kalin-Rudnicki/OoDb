...
trait Node[N] {
  ...
  @tailrec
  protected final def afterInsert(
    key: Long,
    value: Long,
    k: List[Long],
    v: List[Long],
    pK: List[Long],
    pV: List[Long]
  ): Option[(Boolean, List[Long], List[Long])] = ((k, v): @unchecked) match {
    case (Nil, Nil) =>
      (true, (key :: pK).reverse, (value :: pV).reverse).some
    case (hK :: tK, hV :: tV) =>
      if (key < hK)
        (true, pK.reverse ++ (key :: k), pV.reverse ++ (value :: v)).some
      else if (key == hK) {
        if (value == hV)
          None
        else
          (false, pK.reverse ++ k, pV.reverse ++ (value :: tV)).some
      } else
...
object NodeUtils {
  ...
  @tailrec
  def takeAtMax(max: Int,
                done: Boolean,
                stopAt0: Boolean,
                list: List[Long],
                prev: List[Long]): (List[Long], List[Long]) =
    if (max > 0)
      if (done)
  ...
  def takeFirst(c: Int,
                list: List[Long],
                prev: List[Long] = Nil): (List[Long], List[Long]) =
    if (c > 0)
