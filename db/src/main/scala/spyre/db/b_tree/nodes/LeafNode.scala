...
case class LeafNode(pos: Long,
  ...
  def find(key: Long): Option[Long] = {
    @tailrec
    def loop(k: List[Long], v: List[Long]): Option[Long] =
      ((k, v): @unchecked) match {
        case (Nil, Nil) =>
          None
        case (hK :: tK, hV :: tV) =>
  ...
  def remove(key: Long): Option[(Long, LeafNode, Boolean)] = {
    ...
    def loop(k: List[Long],
             v: List[Long],
             pK: List[Long],
             pV: List[Long]): Option[(Long, LeafNode, Boolean)] =
      ((k, v): @unchecked) match {
        case (Nil, Nil) =>
          None
        case (hK :: tK, hV :: tV) =>
  ...
  def borrowFromEnd: (LeafNode, (Long, Long)) = {
    ...
    def loop(k: List[Long],
             v: List[Long],
             pK: List[Long],
             pV: List[Long]): (LeafNode, (Long, Long)) =
      ((k, v): @unchecked) match {
        case (hK :: Nil, hV :: Nil) =>
          (LeafNode(pos, pK.reverse, pV.reverse, nextLeaf), (hK, hV))
        case (hK :: tK, hV :: tV) =>
          loop(tK, tV, hK :: pK, hV :: pV)
      }
...
object LeafNode {
  ...
  def apply(pos: Long, order: Int, list: List[Long]): LeafNode = {
    val (values, nextLeaf :: keyStart) =
      NodeUtils.takeAtMax(order - 1, false, true, list, Nil)
    val (keys, _) =
      NodeUtils.takeAtMax(values.length, false, false, keyStart, Nil)
