...
case class InternalNode(pos: Long, keys: List[Long], children: List[Long])
    extends Node[InternalNode] {
  ...
  def insert(
    ...
    key: Long,
    value: Long
  ): Option[(InternalNode, Option[(Long, InternalNode)])] =
    afterInsert(key, value, keys, children.tail, Nil, children.head :: Nil) match {
      case None =>
        None
      case Some((_, newKeys, newValues)) =>
        if (newKeys.length < order)
          (new InternalNode(pos, newKeys, newValues), None).some
        else {
          val (keys1, passBackVal :: keys2) =
            NodeUtils.takeFirst(order / 2, newKeys)
          val (values1, values2) = NodeUtils.takeFirst(order / 2 + 1, newValues)
          (
            new InternalNode(pos, keys1, values1),
            (passBackVal, new InternalNode(freeListStart, keys2, values2)).some
          ).some
        }
    }
  ...
  /**
	  ...
	  */
  def childAndNeighbors(
    key: Long
  ): (Option[Long], Long, Option[Long], Option[(Long, Long)]) = {
    // @tailrec
    def loop(
      pK: Option[Long],
      k: List[Long],
      ...
      ((k, v): @unchecked) match {
        ...
        case (hK :: tK, hV :: tV) =>
          if (key < hK)
            (pK, pV, pV2, (hK, hV).some)
          else
            loop(hK.some, tK, hV, pV.some, tV)
      }

    loop(None, keys, children.head, None, children.tail)
  }

  def borrowFromEnd: (InternalNode, (Long, Long)) = {
    @tailrec
    def loop(k: List[Long],
             v: List[Long],
             pK: List[Long],
             pV: List[Long]): (InternalNode, (Long, Long)) =
      ((k, v): @unchecked) match {
        case (hK :: Nil, hV :: Nil) =>
          (InternalNode(pos, pK.reverse, pV.reverse), (hK, hV))
        case (hK :: tK, hV :: tV) =>
  ...
  def handle(changes: MMap[Long, P]): Option[InternalNode] = {
    @tailrec
    def loop(anyChanges: Boolean,
             k: List[Long],
             v: List[Long],
             pK: List[Long],
             pV: List[Long]): Option[InternalNode] =
      ((k, v): @unchecked) match {
        case (Nil, Nil) =>
          if (anyChanges)
...
object InternalNode {
  ...
  def apply(pos: Long, order: Int, list: List[Long]): InternalNode = {
    val (children, keyStart) =
      NodeUtils.takeAtMax(order, false, true, list, Nil)
    val (keys, _) =
      NodeUtils.takeAtMax(children.length - 1, false, false, keyStart, Nil)
