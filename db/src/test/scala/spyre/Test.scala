...
object Test {
  ...
  class Conf(arguments: Seq[String]) extends MainConfReqSub(arguments) {
    addSubcommand(new SubConfReqSub("speeds") {
      addSubcommand(new SubConf("INSERT-GET-REMOVE") {
    ...
    addSubcommand(new SubConf("test") {
      ...
      override protected def onExec(): Unit = {
        ...
        try {
          ...
          for (i <- 0.until(s)) {
            ...
            if (act != exp)
              throw new RuntimeException(
                s"Unexpected value for $k: Expected: $exp, Actual: $act"
              )
            if (bTree.size != map.size)
              throw new RuntimeException(
                s"BTree has invalid size: ${bTree.size} != ${map.size}"
              )
          }
          println(s"=====> $s")
          println
          ...
          for ((k, v) <- pairs) {
            ...
            if (act2.isDefined)
              throw new RuntimeException(
                s"Value was still found after deletion: $k => $act2"
              )
            if (bTree.size != map.size)
              throw new RuntimeException(
                s"BTree has invalid size: ${bTree.size} != ${map.size}"
          ...
          if (bTree.size != 0)
            throw new RuntimeException(
              s"BTree was not empty in the end (${bTree.size})"
            )
        } catch {
          case e: Exception =>
            // bTree.showStats
    ...
    addSubcommand(new SubConf("misc") {
      override protected def onExec(): Unit =
        Misc.test
    })
  }
